...
- `src/` — исходники:
  ...
  - `src/shared/` — переиспользуемые куски внутри приложения (без привязки к конкретной странице):
    - `src/shared/api/` — axios client + типы ошибок + маппинг ответов.
      - `src/shared/api/generated/` — сгенерированный код/типы (например по OpenAPI). Ручные правки запрещены, только регенерация.
    - `src/shared/ws/` — WebSocket client + типы сообщений + обработчики (если используется).
      - `src/shared/ws/generated/` — сгенерированные типы сообщений по AsyncAPI (если используется). Ручные правки запрещены, только регенерация.
