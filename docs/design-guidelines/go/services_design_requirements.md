...
- `internal/transport/http/generated/` — сгенерированный Go-код по OpenAPI (только `**/generated/**`, руками не правим).
- `internal/transport/grpc/generated/` — сгенерированный Go-код по `.proto` (только `**/generated/**`, руками не правим).
- `api/server/generated/**` — сгенерированные артефакты контрактов (например OpenAPIv2 из proto для dev).
...
- Graceful shutdown (серверы/коннекты закрываются корректно):
  - `cmd/<service>/main.go` перехватывает `SIGINT`, `SIGTERM`, `SIGQUIT`, `SIGHUP` и отменяет `ctx` приложения;
  - при любом раннем `return err` обязаны закрываться уже открытые ресурсы (коннекты/пулы/серверы/otel).
