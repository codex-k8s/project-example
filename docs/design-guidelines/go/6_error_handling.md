# 8 — Обработка ошибок (обязательные правила)
...
- Для Go HTTP используем `github.com/getkin/kin-openapi` (загрузка спеки + request validation), а ошибки валидатора нормализуем в единый безопасный контракт.
