...
Правило:
- Любая фоновая доставка событий между транспортами (например, gRPC stream -> WS broadcast) реализуется в transport-слое (например, `internal/transport/ws/*`), а `internal/app` только запускает этот процесс и управляет жизненным циклом через `ctx`.
