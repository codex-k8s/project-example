...
- `deleted_at` (`timestamptz`, `NULLABLE`, индекс):
  - время мягкого удаления сообщения (если установлено, сообщение скрывается из выдачи).
...
- удаление сообщений реализовано как мягкое удаление
  через заполнение `deleted_at` (сообщения не возвращаются в API).
