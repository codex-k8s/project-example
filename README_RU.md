...
- `CODE_ROOT_BASE` — базовый каталог для исходников dev‑AI слотов и staging‑копии репозитория (пример: `/home/runner/codex/envs`):
  - dev‑AI слоты: `${CODE_ROOT_BASE}/<slot>/src`;
  - staging: `${CODE_ROOT_BASE}/staging/src`;
  например `/home/runner/codex/envs/`;
- `DATA_ROOT` — каталог с данными БД/Redis (пример: `/home/runner/codex/data`);
...
3. При следующем push в `main`:
   ...
   - исходники будут синхронизированы в `${CODE_ROOT_BASE}/staging/src` и примонтированы в staging‑подах;
