...
3. При следующем push в `main`:
   - соберутся и отзеркалятся необходимые образы;
   - `codexctl apply --env staging` применит инфраструктуру и сервисы;
...
3. Запустится workflow `.github/workflows/ai_plan_issue.yml`:
   ...
   - развернёт инфраструктуру и сервисы через `codexctl manage-env ensure-ready`;
...
2. Workflow `ai_dev_issue.yml`:
   - выделит/найдёт слот через `manage-env ensure-slot`;
   - развернёт окружение в этом слоте (`ensure-ready --prepare-images --apply`);
