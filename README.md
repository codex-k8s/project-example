⚠️ В этом репозитории находится проект в активной стадии разработки ⚠️
> **пуши в мастер**, нестабильность, резкие повороты  все это будет тут еще некоторое время.
...
- сервисы:
  - `chat-web` — SPA‑фронтенд на Vue3 + Pinia (логин/регистрация/сообщения);
  - `chat-gateway` — внешний Go‑gateway: REST + WebSocket, cookie‑сессии, интеграция внутрь по gRPC;
  - `users` — internal Go‑сервис пользователей и паролей (Postgres);
  - `messages` — internal Go‑сервис сообщений (Postgres) + gRPC‑stream событий;
  - `messages-cleaner` — jobs‑cron, удаляет сообщения старше 10 минут;
- автоматизация:
...
Стартовая точка — `docs/design-guidelines/AGENTS.md`. Там навигация по обязательным гайдам.
Дальше используйте профильные документы из `docs/design-guidelines/common/**`,
`docs/design-guidelines/go/**`, `docs/design-guidelines/vue/**` — в зависимости от задачи.
...
- `services/` — сервисы по зонам (`external|internal|jobs|staff|dev`):
  - `services/external/chat-web` — SPA‑фронтенд чата (Vue3 + Pinia);
  - `services/external/chat-gateway` — внешний gateway (REST + WebSocket);
  - `services/internal/users` — пользователи и пароли (Postgres);
  - `services/internal/messages` — сообщения (Postgres) + gRPC‑stream событий;
  - `services/jobs/messages-cleaner` — cron‑задача очистки старых сообщений;
...
## 10. Что дальше поправить

В проекте уже есть обязательные гайды в `docs/design-guidelines/**`, но остальная
документация требует актуализации. Что стоит сделать дальше:

- добавить актуальные документы по архитектуре и доменам сервиса (сейчас в `docs/` есть только `design-guidelines/`);
- описать модели данных и правила миграций под новые сервисы `users/messages`;
- описать контракты и интеграции (gRPC/OpenAPI/WebSocket/AsyncAPI) для `chat-gateway`;
- актуализировать разделы про наблюдаемость и общие библиотеки под текущую структуру.
...
локально или в кластере с жёстким сетевым ограничением доступа
(NetworkPolicy/Ingress ACL), чтобы к нему не было публичного доступа.
